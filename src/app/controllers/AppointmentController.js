...
class AppointmentController {
  ...
  async store(req, res) {
     ...
     */
    const checkeAvailabity = await Appointment.findOne({
      where: {
        provider_id,
        canceled_at: null,
        date: hourStart,
    ...
    const formattedDate = format(hourStart, "'dia' dd 'de' MMM', ás' H:mm'h'", {
      locale: pt,
    });
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You can only cancel appointments 2 hours id advance!',
      });
    }
